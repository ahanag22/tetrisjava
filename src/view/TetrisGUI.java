...
public class TetrisGUI  extends JFrame implements Observer {
    ...
    */
    ...
    private void keyEventHandler(final KeyEvent theEvent) {
        ...
        if (!handleViaUpDown(key)) {         
            if (myIsNotPause && key == KeyEvent.VK_END) {
                callHardDrop();                  
    ...
    /**
     * Check if the event can be handled via direction keys.
     * @param theKey the key.
     * @return flag indicates if the key is handled.
     */
    private boolean handleViaUpDown(final int theKey) {
        boolean toRet = false;
        if (myIsNotPause && theKey == KeyEvent.VK_LEFT) {
            callLeft(); 
            toRet = true;
        } else if (myIsNotPause && theKey == KeyEvent.VK_RIGHT) {
            callRight();
            toRet = true;
        } else if (myIsNotPause && theKey == KeyEvent.VK_UP) {               
            callUp();
            toRet = true;
        } else if (myIsNotPause && theKey == KeyEvent.VK_DOWN) {
            callDown(); 
            toRet = true;
        }
